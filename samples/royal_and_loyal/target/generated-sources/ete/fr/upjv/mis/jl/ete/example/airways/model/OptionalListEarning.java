 ...
 * @author jldeleage
 */
public class OptionalListEarning extends LinkedList<OptionalEarning> {
     ...
     * 
     */
    public List<Earning> get() throws InstantiationException {
        if (isPresent()) {
            List<Earning>    result = FactoryMethods.newList(Earning.class);
            for (OptionalEarning item : this) {
                result.add(item.get());
            }
            return result;
        }
